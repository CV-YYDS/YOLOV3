        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, (imgs, label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes)  in enumerate(self.train_dataloader):
                ...
                if i%100==0:
                    ...
                    print(s)
                    print(('loss_giou: %.4f    loss_conf: %.4f    loss_cls: %.4f    loss: %.4f    lr: %g') % (mloss[0],mloss[1], mloss[2], mloss[3],self.optimizer.param_groups[0]['lr']),file = f0)
            ...
            self.scheduler.step()
            print(('loss_giou: %.4f    loss_conf: %.4f    loss_cls: %.4f    loss: %.4f    lr: %g') % (mloss[0],mloss[1], mloss[2], mloss[3],self.optimizer.param_groups[0]['lr']),file = f1)
